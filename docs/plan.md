...
| NC5 flex (waves 1-5b + 6b + reduced-dof of 6) | **DONE**; ratchet 239 | flexcomp procedural + elasticity (young>0) + direct/gmsh/mesh-file expansion, edge+vert flex equalities, and radial/2d reduced-dof all compile natively bit-identical. Waves 5 (gmsh: LoadGMSH41/22 ascii+binary lifted), 5b (mesh-file: LoadMeshRaw load-only entry + MakeMesh dim 1/2/3 tets), 6b (vert equality → mjEQ_FLEXVERT), 6-partial (radial one-slider/2d two-slider per vertex, degenerate radial center pinned). Remaining flex descope: **full interpolated FE (trilinear/quadratic) + strain equality** (nodal stiffness stack, user_mesh.cc:3826-4500; ~12 files) — see HANDOFF.md queue item 1 |
| NC5 remaining (file textures/hfields, skins, attach, muscle/LR) | queued | fallbacks all reasoned; interpolated-FE flex is the largest single remaining family; recon complete (see HANDOFF.md queue) |
...
| XML-route 100% parity (engine plugins) | **DONE** | Harness registers the first-party engine plugins (`mujoco.elasticity.*`, `mujoco.sdf.*`, `mujoco.sensor.touch_grid`, `mujoco.pid`) via `mj_loadPluginLibrary` at startup (shared `protospec/lib/harness/plugin_registry.{h,cc}` linked into mj_model_diff/ps_native_diff/ps_compile; DLLs beside mujoco.dll, `--plugin-dir`/`PROTOSPEC_PLUGIN_DIR` override). The 17 previously-skipped plugin models (18 references; 3 share elasticity.cable) now round-trip byte-identical — IO wave 3 preserves `<extension>`/`<plugin>` config as ordered data. **differential 376/387 identical, 0 differ**; the 11 remaining skips are the only models MuJoCo itself cannot load standalone (10 deliberately-malformed flexcomp fixtures + 1 sleep-init engine-fail fixture). Parity floor enforced in `tests/test_differential.py::test_xml_parity_floor`. bridge corpus: 377 compiled, 0 fail. |
| Canonicalization (minimal representation) | **Wave A + Wave B DONE** | Wave A: orientation family canonicalized at read (orientation→`quat`, inertia→`diaginertia`+`iquat`, CameraIntrinsics variant dissolved R1/R2; `core::Resolve*` lifted+registered). Wave B: light `directional`→`type`, tendon `springlength` scalar→pair (R3 arity unify), cylinder `diameter`→`area` (π/4 d²), material `texture`→`<layer role="rgb">` (element-level input alias), numeric `size`→materialized data, keyword-set order→enum order, R1 camera-intrinsic tier-3 lint. differential 359/387 + native ratchet 201 held; pytest 1625/120; cpp ctest 6/6; studio 5/5. Inventory + waves: docs/plan_canonicalization.md (3 flags owner-approved 2026-07-13) |
...
`uv run pytest` (Python) and cmake+ctest under protospec/lib/ (VS2022 generator; MUJOCO_ROOT points at the
vendored prebuilt 3.10.0 in the UE plugin's third_party/MuJoCo/src). supported.json under protospec/lib/io/
