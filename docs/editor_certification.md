...
`attic/studio_host/test/`, parked with the standalone host); the parenthesised check
counts are the current identical totals.
...
| # | Action | Expected | P/F |
|---|---|---|---|
| M1 | Launch as above | Studio window, **two-panel default layout**: Hierarchy left, Viewport center, Details right; **Assets and Diagnostics are NOT standing panels** (Assets folds into the Hierarchy; Diagnostics into the status-bar error chip -- both still toggleable from the Plugins/View menu); humanoid rendered at qpos0; old Explorer/Editor spec tabs absent; status bar shows compile path + time; title has no dirty marker | ☐ |
...
| M41 | In the Hierarchy, right-click the **Assets** section (or the panel-header **+ Asset** button) → **New Texture** | A default `<texture>` asset is **instant-created** (uniquely named, no modal) and appears in the Hierarchy's Assets section; selecting it edits its builtin / rgb1 / rgb2 / width / height in **Details**; the model recompiles (`ntex` rises for a builtin) | ☐ |
| M42 | In the Hierarchy, right-click the **Assets** section (or **+ Asset**) → **New Material** | A default `<material>` asset is **instant-created** and appears in the Assets section; selecting it edits rgba / specular / shininess / metallic / roughness in **Details**; the model recompiles (`nmat` rises) | ☐ |
