...
def generate(schema_path: str = SCHEMA) -> dict[str, str]:
    ...
    # The ProtoSpec->mjSpec field-mapping layer (Wave 1 of the mjSpec shim) is
    # generated by the same invocation into the same lib/generated/ tree, and its
    # coverage/drift gate runs here so `emit --check` fails on any unmapped field.
    ...
    files.update(emit_mjs.generate_files(schema=s.doc))
    return files
...
def main() -> None:
    ...
    if args.check:
        ...
        if stale:
            ...
            sys.exit(1)
        print(
            f"lib/generated/ is up to date ({len(files)} files); "
            "lib/python/generated/ is up to date"
        )
        return
