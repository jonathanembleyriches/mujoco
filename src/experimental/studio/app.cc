...
void App::HandleKeyboardEvents() {
  ...
  // shortcuts (e.g. the editor's undo/save/transport bindings). Moved ahead of
  // the built-in chords from upstream's trailing else-branch dispatch.
  bool handled = false;
  ...
  // NOTE: the plugin KeyHandler dispatch that upstream keeps here as a trailing
  // `else` now runs FIRST (top of this function) so editor bindings shadow the
  // built-in chords.
}
