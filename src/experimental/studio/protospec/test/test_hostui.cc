...
//
// The editor cluster registers through mujoco::platform::RegisterPlugin, whose
// real definition lives in the (SDL/Filament-heavy) platform library. To stay
// windowless we provide a minimal registry of our own here and instantiate it
// for the plugin types the viewport editor registers; nothing else changes. The
// registry dedups by name, so the viewport editor is registered ONCE against one
// shared EditorContext, which every scenario below reuses.
...
// --- Minimal plugin registry (windowless stand-in for platform/ux/plugin.cc) - //
...
void RegisterPlugin(T plugin) {
  auto& t = Table<T>();
  for (const T& e : t) {
    if (e.name && plugin.name && std::strcmp(e.name, plugin.name) == 0) return;
  }
  t.push_back(plugin);
}
template <typename T>
void ForEachPlugin(const std::function<void(T*)>& fn) {
  for (T& p : Table<T>()) fn(&p);
}
template void RegisterPlugin<KeyHandlerPlugin>(KeyHandlerPlugin);
template void RegisterPlugin<ViewportPlugin>(ViewportPlugin);
template void RegisterPlugin<ViewportGuiPlugin>(ViewportGuiPlugin);
template void RegisterPlugin<OverlayPlugin>(OverlayPlugin);
template void ForEachPlugin<KeyHandlerPlugin>(
    const std::function<void(KeyHandlerPlugin*)>&);
template void ForEachPlugin<ViewportPlugin>(
    const std::function<void(ViewportPlugin*)>&);
}  // namespace mujoco::platform

namespace platform = mujoco::platform;
