...
// The MJCF grammar table is generated from protospec/schema/mujoco.spec by
// protospec_gen.emit_native (see protospec/README.md). It defines both the
// MJCF[] table and nMJCF_GENERATED (its row count).
#include "xml_native_schema.inc"
...
// constructor
mjXReader::mjXReader() : schema(MJCF, nMJCF_GENERATED) {
...
void mjXReader::Compiler(XMLElement* section, mjSpec* s) {
  ...
  MapValue(section, "conflict", &s->compiler.conflict, conflict_map,
           conflict_sz);
