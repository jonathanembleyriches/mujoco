...
// Lifted mjuu_* implementation. See mjuu_util.h for provenance and the
// adaptation notes. Registry entry "user_util_cc". Inside the MuJoCo
// quarantine zone (includes mujoco.h).
...
// engine/engine_crossplatform.h is engine-internal; the only symbol
// used from it here is strcasecmp. Provide the portable shim.
#if defined(_WIN32)
...
template std::vector<unsigned char> StringToVector(char* cs);
