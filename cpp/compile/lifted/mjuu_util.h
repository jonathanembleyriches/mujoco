...
// Lifted from MuJoCo: the mjuu_* free-standing user-layer math/util pool
// (design ledger 3.2, tier-2 reuse). See NOTICE for attribution.
//
//   Upstream: src/user/user_util.h  (whole file, zero mjC* references)
//   Pin:      mjVERSION_HEADER 3010000 (MuJoCo 3.10.0)
//   License:  Apache-2.0, (c) DeepMind Technologies Limited.
//   Registry: snapshots/lifted_code.json entry "user_util_h".
//
// Adaptation (registry notes): symbols are moved into
// ps::mjcf::compile::lifted so they never clash with the mujoco.dll exports;
// MJAPI export markers are stripped (this is a static lib, not the engine DLL).
// Algorithm, constants (mjEPS/mjMINMASS), and signatures are verbatim.

#ifndef PROTOSPEC_COMPILE_LIFTED_MJUU_UTIL_H_
#define PROTOSPEC_COMPILE_LIFTED_MJUU_UTIL_H_
...
template<> std::vector<std::string> StringToVector(char* cs);
...
#endif  // PROTOSPEC_COMPILE_LIFTED_MJUU_UTIL_H_
